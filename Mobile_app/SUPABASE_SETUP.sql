...
-- 4. user_roles RLS (explicit per-command — prevents privilege escalation)
CREATE POLICY "Users can view their own roles" ON public.user_roles FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Admins can view all roles"    ON public.user_roles FOR SELECT USING (public.has_role(auth.uid(), 'admin'));
CREATE POLICY "Admins can insert roles"      ON public.user_roles FOR INSERT WITH CHECK (public.has_role(auth.uid(), 'admin'));
CREATE POLICY "Admins can update roles"      ON public.user_roles FOR UPDATE USING (public.has_role(auth.uid(), 'admin')) WITH CHECK (public.has_role(auth.uid(), 'admin'));
...
-- 6. Daily logs (includes supplements_taken JSONB)
CREATE TABLE public.daily_logs (
    ...
    supplements_taken JSONB NOT NULL DEFAULT '{}',
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    UNIQUE (user_id, date)
);
...
CREATE POLICY "Users can view their own meal entries"   ON public.meal_entries FOR SELECT USING (EXISTS (SELECT 1 FROM public.daily_logs WHERE daily_logs.id = meal_entries.daily_log_id AND daily_logs.user_id = auth.uid()));
CREATE POLICY "Users can create their own meal entries" ON public.meal_entries FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM public.daily_logs WHERE daily_logs.id = meal_entries.daily_log_id AND daily_logs.user_id = auth.uid()));
CREATE POLICY "Users can update their own meal entries" ON public.meal_entries FOR UPDATE USING (EXISTS (SELECT 1 FROM public.daily_logs WHERE daily_logs.id = meal_entries.daily_log_id AND daily_logs.user_id = auth.uid()));
CREATE POLICY "Users can delete their own meal entries" ON public.meal_entries FOR DELETE USING (EXISTS (SELECT 1 FROM public.daily_logs WHERE daily_logs.id = meal_entries.daily_log_id AND daily_logs.user_id = auth.uid()));

-- 8. User settings (includes supplements JSONB)
...
-- 10. Timestamp update function + triggers
...
-- 11. Auto-create user settings on signup
