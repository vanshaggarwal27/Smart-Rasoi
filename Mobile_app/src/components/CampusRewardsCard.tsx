...
import { Utensils, ChevronRight, GraduationCap } from "lucide-react";
import { Link } from "react-router-dom";

export const CampusRewardsCard = ({ variant = "default" }: { variant?: "default" | "compact" }) => {
  ...
  if (variant === "compact") {
    return (
      <Link to="/rewards" className="group flex items-center gap-3 bg-card/50 backdrop-blur-sm border border-primary/10 rounded-2xl p-3 hover:bg-primary/5 transition-all active:scale-95 shadow-sm">
        <div className="flex h-10 w-fit px-2 items-center justify-center rounded-xl bg-primary/20 text-primary group-hover:scale-110 transition-transform">
          <span className="text-xs font-black italic tracking-tighter whitespace-nowrap pr-0.5">Tier {levelInfo.level}</span>
        </div>
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-card to-card/50 backdrop-blur-xl border-2 border-primary/10 rounded-[2rem] p-5 shadow-xl transition-all hover:border-primary/30 hover:shadow-primary/5 active:scale-[0.98]">
        ...
        <div className="relative flex items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <h3 className="text-xl font-black italic text-foreground tracking-tight uppercase">Campus Rewards Tier {levelInfo.level}</h3>
          </div>
        ...
        <div className="mt-5 space-y-2">
          <div className="flex justify-between text-[10px] uppercase font-bold text-muted-foreground tracking-wider">
            <span>Next Privilege Unlock</span>
            <span className="text-primary">{levelInfo.xpInLevel} / {levelInfo.xpTarget} CP</span>
          </div>
        ...
        <div className="mt-4 flex items-center justify-center text-[9px] font-black uppercase text-primary/50 tracking-[0.3em] group-hover:text-primary/100 transition-colors">
          View Reward Perks <ChevronRight size={10} className="ml-1" />
        </div>
      </div>
    </Link>
  );
